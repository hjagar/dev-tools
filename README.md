...
| `hdv-release-project.ps1` | Runs a 5-step automated release pipeline for Node.js projects (Windows) |
| `hdv-release-project.sh` | Same, for Linux/Mac |
...
To automate releases for your own projects, use the `hdv-release-project` command. It supports Node.js projects out of the box.

Run from the root of your project:

```powershell
# Windows
hdv-release-project.ps1 -ReleaseType patch # or minor / major
```
```bash
# Linux / Mac
hdv-release-project.sh patch # or minor / major
```

Options:
- `-LocalOnly` (PS) / `--local-only` (Bash): Skip git push and GitHub release.
- `-SkipTest` (PS) / `--skip-test` (Bash): Bypass running tests.
- `-SkipBuild` (PS) / `--skip-build` (Bash): Bypass running build.

The pipeline runs 5 steps in order:
1. **Pre-flight checks**: Verifies git working tree is clean and `package.json` exists.
2. **Quality Gate**: Runs `npm test` and `npm run build` (if defined in `package.json` and not skipped).
3. **Version Bump**: Runs `npx standard-version` to update files, changelog, and tag the commit.
4. **Git Push**: Pushes tag and commits to the remote branch (unless `-LocalOnly` is set).
5. **GitHub Release**: Creates the release on GitHub with autogenerated changelog (unless `-LocalOnly` is set).
...
| `hdv-release-project.ps1` | Corre un pipeline automatizado de 5 pasos para releases de proyectos (Windows) |
| `hdv-release-project.sh` | Lo mismo, para Linux/Mac |
...
Para automatizar los lanzamientos de tus propios proyectos, usá el comando `hdv-release-project`. Soporta proyectos Node.js por defecto.

Corrés el comando desde la raíz de tu proyecto:

```powershell
# Windows
hdv-release-project.ps1 -ReleaseType patch # o minor / major
```
```bash
# Linux / Mac
hdv-release-project.sh patch # o minor / major
```

Opciones:
- `-LocalOnly` (PS) / `--local-only` (Bash): Omite el push a remoto y la release en GitHub.
- `-SkipTest` (PS) / `--skip-test` (Bash): Saltea la ejecución de pruebas.
- `-SkipBuild` (PS) / `--skip-build` (Bash): Saltea la ejecución de compilación.

El pipeline ejecuta 5 pasos en orden:
1. **Pre-checks**: Valida que Git esté limpio y que exista `package.json`.
2. **Control de Calidad**: Ejecuta `npm test` y `npm run build` (si están definidos en `package.json` y no fueron omitidos).
3. **Bump de Versión**: Ejecuta `npx standard-version` para actualizar versión, changelog y taggear.
4. **Git Push**: Sube los commits y tags a remoto (salvo que uses `-LocalOnly`).
5. **Release en GitHub**: Publica el release en GitHub con notas autogeneradas (salvo que uses `-LocalOnly`).
